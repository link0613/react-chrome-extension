(function(exports, global) {
    ...
    var BI = W.Site.Common.BI || function() {
        ...
        var path = Utils.getLocationOrigin().indexOf("wix.com") > -1 ? "//frog.wix.com/" : "//frog.wixpress.com/";
    ...
    W.Site = W.Site || {};
    W.Site.Common = W.Site.Common || {};
    W.Site.Common.SUPPORTED_LANGUAGES = [ "en", "de", "es", "fr", "it", "nl", "no", "pl", "pt", "ru", "sv", "ja", "ko", "tr", "da", "hi" ];
    W.Site.Common.DefaultLayout = W.Site.Common.DefaultLayout || function() {
        var noop = function() {};
        function initMarketingHeaderAndFooter(options) {
            document.querySelectorAll("#wix-menu, #wix-footer").forEach(function(node) {
                node.innerHTML = "";
            });
            var config = {
                baseUrl: W.Site.Common.clientTopology.headerAndFooterStaticsUrl + "bower_components/marketing-header-and-footer/statics/"
            };
            if (options.forceMobileView && options.forceMobileView.force) {
                config.forceView = "mobile";
            }
            var hostPage = null;
            ...
            W.Site.Common.marketingHeaderAndFooter.init(config);
            return;
        }
        ...
        function getPostSignUpUrlByVertical() {
            ...
            if (userVerticalId) {
                ...
                return Utils.getLocationOrigin() + "/new/vertical?referralAdditionalInfo=header";
            }
        }
                ...
                loadExperiments(options, function() {
                    if (experiments["specs.marketing.NewHeaderAndFooter"] === "true") {
                        if (W.Site.Common.marketingHeaderAndFooter) {
                            initMarketingHeaderAndFooter(options);
                            return;
                        } else {
                            console.warn("Old header and footer tries to init new one but it does not loaded");
                        }
                    }
                    ...
                    if (experiments["specs.CzechLanguageInMenu"] === "true") {
                        options.supportedLanguages.push("cs");
                    }
    ...
    var MobileMenu = W.Site.Common.MobileMenu || function() {
        ...
        function events() {
            ...
            jQuery(".wm-mobile-menu-links a").on("click tap touch", function(e) {
                e.preventDefault();
    ...
    var MobileMenuLogin = W.Site.Common.MobileMenuLogin || function() {
        ...
        function gotoLoginPage() {
            setTimeout(function() {
                window.open("https://" + window.location.hostname + "/signin", "_top");
            }, 500);
        }
        function events() {
            jQuery("#wm-mobile-get-started").on("click tap touch", function(e) {
                e.preventDefault();
    ...
    var View = function() {
        ...
        function drawMobileOptimizedMenuHandle() {
            if (options.mobileOptimized || options.forceMobileView.force) {
